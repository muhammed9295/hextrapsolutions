        ...
        <>
            {blogs.slice(pageVisited, pageVisited + blogsPerPage).map((data) => (
                <div className="blog-grid" key={data.id}>
                    <h3 className="title">
                        <Link href={`/blog-details/${data.id}`}><a className='text-decoration-none'>{data.title}</a></Link>
                    </h3>
                    <div className="author">
                        ...
                        <div className="info">
                            <h6 className="author-name">
                                <Link href={`/archive/${slugify(data.author_name)}`}><a className='text-decoration-none'>{data.author_name}</a></Link>
                            </h6>
                            ...
                            <Slider {...slideSettings} className="slick-arrow-nav">
                                {data.large_thumb.map((data, index) => (
                                    <div className="slide-item" key={index}>
                                        <img src={`${data}`} alt="Blogs" />
                                    </div>
                                ))}
                                
                            </Slider> 
                            : <Link href={ `/blog-details/${data.id}`}><a className='text-decoration-none'><img src={`${data.large_thumb}`} alt="Blog" /></a></Link>
                        }
                    ...
                    <p>{data.excerpt}</p>
                    <Link href={`/blog-details/${data.id}`}><a className='text-decoration-none axil-btn btn-borderd btn-large'>Read More</a></Link>
                </div>
            ))}
